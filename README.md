...
Phase 5H also has a separate already-stopped legacy cold-activation operator. It never substitutes for the authenticated shutdown route when a process or listener exists. Core requires exact `venice-media-local:activate-release-slot` verification, persists two authoritative zero-work samples, issues a binding-exact authorization for at most 60 seconds, and consumes it once while the Windows operator holds the exclusive transition lock. See `docs/specs/phase5h-cold-activation-remediation-2026-07-14.md` and `scripts/Invoke-Phase5HColdActivation.ps1`.
