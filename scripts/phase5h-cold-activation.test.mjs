...
test('foreground operator keeps both credentials off arguments, environment, logs, hashes, and evidence', () => {
  ...
  assert.match(operator, /operator-handoff/)
  assert.match(operator, /normal-human-web-session/)
  assert.match(operator, /Test-TransientClaimTransportFailure/)
  assert.match(operator, /maxConsecutiveClaimTransportFailures\s*=\s*5/)
  assert.match(operator, /Start-Sleep -Milliseconds \$backoffMs/)
  assert.match(operator, /retrying the same handoff/)
  assert.equal((operator.match(/operator-handoff\/begin/g) || []).length, 1)
  assert.match(operator, /entering the code alone did not complete the handoff/i)
  assert.match(operator, /exactActionAccepted/)
  assert.match(operator, /buildAuthorityAccepted/)
  assert.match(operator, /buildAuthority\.expiresAt/)
  assert.match(operator, /-not \$exactActionAccepted -and -not \$buildAuthorityAccepted/)
  assert.match(operator, /RedirectStandardInput\s*=\s*\$true/)
  assert.match(operator, /StandardInput\.WriteLine\(\$transport\)/)
  assert.match(operator, /agent-control-token\.venice-media-local/)
  assert.doesNotMatch(operator, /\/api\/auth\/desktop-connector/)
