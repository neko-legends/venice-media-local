...
Windows PowerShell 5.1 does not expose `ProcessStartInfo.StandardInputEncoding`, so its redirected standard input can begin with a UTF-8 byte-order mark. The candidate defensively removes only that leading BOM plus transport whitespace before constructing the authorization header; this prevents the stream marker from turning an otherwise valid bearer into a false 401 without creating any credential fallback.
